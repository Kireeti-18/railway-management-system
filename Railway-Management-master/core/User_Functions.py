...
def AvailableTrains():
    mn = mysql.connector.connect(host="localhost", user='root',password='root', database="railway")
    cur = mn.cursor()
    ...
    cur.execute(
        'SELECT Train_No, Source_Station_Name, Destination_Station_Name, Arrival_Time, Departure_Time from train_info where Source_Station_Code="{}" AND Destination_Station_Code="{}";'.format(start_opt, final_opt))
    result = cur.fetchall()
    os.system("cls")
    time.sleep(1)
...
def CheckFare():
    ...
    if len(result_fare) >= 10:
        try:
            ...
            for x in range(ask):
                y = result_fare[x]
                print(result_fare[x], "Rs.", int(y[1]) * sleeper_charge, "Rs.", int(y[1]) * third_ac_charge, "Rs.",int(y[1]) * second_ac_charge, "Rs.", int(y[1]) * first_ac_charge, "\n")
        ...
        for x in result_fare:
            print(x, "Rs.", int(x[1]) * sleeper_charge, "Rs.", int(x[1]) * third_ac_charge, "Rs.",int(x[1]) * second_ac_charge, "Rs.", int(x[1]) * first_ac_charge, "\n")
...
def ShowBookings():
    ...
    if len(result) == 0:
        ...
        booking_no = 1
        print(["Train_No", "Passenger_Name", "Mobile_No","Passenger_Adhaar", "Time_Of_Booking", "Booking_ID", "Class"])
        for x in result:
...
def BookTrain():
    ...
    while True:
        ...
        if ask in ["Y", "y"]:
            print("Booking...")
            try:
                query = "INSERT INTO bookings values({}, '{}', '{}', '{}', '{}', {}, '{}')".format(train_no, Name, Mobile, adhaar, date, id, Class)
                cur.execute(query)
            except DataError:
        ...
        try:
            ...
            if len(str(unique_id)) == 0:
                ...
                if len(result) == 0:
                    print("No Records Found!")
                    break
                print(["Train_No", "Passenger_Name", "Mobile_No","Passenger_Adhaar", "Time_Of_Booking", "Booking_ID"])
                for x in result:
                    print(x)
                while True:
